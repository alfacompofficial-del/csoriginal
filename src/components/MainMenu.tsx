      ...
      <div className="relative z-10 min-h-screen flex">
        ...
        <div className="flex-1 flex flex-col p-6">
          ...
          <div className="flex items-center gap-4 mb-auto">
            <button onClick={() => setShowPlayMenu(true)} className="cs-btn-primary text-xl px-12 py-4 animate-pulse-glow">
              ИГРАТЬ
            </button>
            <button onClick={() => setShowSettings(true)} className="group relative p-3 bg-secondary rounded-lg border border-border hover:border-primary/30 transition-all" title="Настройки">
              ...
              <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-xs text-foreground bg-card px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Настройки</span>
            </button>
            <button onClick={() => setShowQuitConfirm(true)} className="group relative p-3 bg-secondary rounded-lg border border-border hover:border-destructive/30 transition-all" title="Выход из игры">
              <LogOut className="w-6 h-6 text-muted-foreground group-hover:text-destructive transition-colors" />
              <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-xs text-foreground bg-card px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Выход из игры</span>
            </button>
          </div>
        ...
        <div className="w-72 cs-panel m-4 p-4 flex flex-col animate-slide-in-right">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowAddFriend(true)} className="p-1.5 rounded hover:bg-secondary transition-colors" title="Добавить друга">
          ...
          <div className="flex-1 overflow-y-auto space-y-1">
            {friends.sort((a, b) => (b.online ? 1 : 0) - (a.online ? 1 : 0)).map((friend) => (
              <div key={friend.name} className="flex items-center gap-3 p-2 rounded hover:bg-secondary/50 transition-colors cursor-pointer">
                <div className={`w-2 h-2 rounded-full ${friend.online ? "bg-cs-success" : "bg-muted-foreground/30"}`} />
                <span className={`text-sm ${friend.online ? "text-foreground" : "text-muted-foreground"}`}>{friend.name}</span>
              </div>
            ))}
          </div>
        </div>
